...
partial model partial_StateSpace "State space model with bus inputs"
  ...
  parameter Integer nQConvGai = 0 "Number of convective heat flow gain inputs";
  parameter Integer nQRadGai = 0 "Number of convective heat flow gain inputs";
  parameter Integer[nWin] winNLay = fill(3,nWin) "Number of window layers";
  parameter Integer nWin=5 "Number of windows";
...
protected
 ...
 final parameter Integer[nWin] offWinCon = {nEmb+nQConv+nQRad+nQConvGai+nQRadGai + sum(winNLay[1:i-1]) + 2*(i-1) for i in 1:nWin}
...
public
  ...
  parameter Real x_start[states]=zeros(states)
    ...
    input IDEAS.Buildings.Linearisation.Interfaces.WindowBus[nWin] winBus(nLay=
        winNLay) annotation (Placement(transformation(
        extent={{-20,-20},{20,20}},
        rotation=90,
        origin={-100,80})));
...
equation
  for i in 1:nEmb loop
    if debug then
      connect(const1_0.y,stateSpace.u[i]);
    else
  ...
  for i in 1:nQConv loop
    if debug then
      connect(const1_0.y,stateSpace.u[nEmb+i]);
    else
      connect(Q_flowConv[i],stateSpace.u[nEmb+i]);
    end if;
  end for;
  for i in 1:nQRad loop
    if debug then
      connect(const1_0.y,stateSpace.u[nEmb+nQConv+i]);
    else
  ...
  for i in 1:nQConvGai loop
    if debug then
      connect(const1_0.y,stateSpace.u[nEmb+nQConv+nQRad+i]);
    else
      connect(Q_flowConvGai[i],stateSpace.u[nEmb+nQConv+nQRad+i]);
    end if;
  end for;
  for i in 1:nQRadGai loop
    if debug then
      connect(const1_0.y,stateSpace.u[nEmb+nQConv+nQRad+nQConvGai+i]);
    else
      connect(Q_flowRadGai[i],stateSpace.u[nEmb+nQConv+nQRad+nQConvGai+i]);
    end if;
  end for;
