...
model StructureH "Standaard woning de schipjes"
  ...
  IDEAS.Buildings.Linearisation.Components.LinWindow slaapkamerRaam(
    ...
    // Linearisation
      IDEAS.Utilities.IO.heatPortPrescribedHeatFlow h2sCon[nZones] if linearise
...
equation
  ...
  connect(nachthal.gainCon, heatPortCon[6]) annotation (Line(points={{52,-217},
          {68,-217},{68,-218},{88,-218},{88,16},{110,16},{110,28.3333},{100,
          28.3333}},
        color={191,0,0}));
  connect(nachthal.gainRad, heatPortRad[6]) annotation (Line(points={{52,-220},
          {76,-220},{92,-220},{92,-11.6667},{100,-11.6667}},color={191,0,0}));
  connect(h2sCon.port1, heatPortRad)
    annotation (Line(points={{120,-20},{100,-20},{100,-20}}, color={191,0,0}));
  connect(h2sRad1.port1, heatPortCon)
    annotation (Line(points={{120,20},{100,20},{100,20}}, color={191,0,0}));
  connect(h2sCon.Q_flow, QFlowCon) annotation (Line(points={{140.8,-13},{146.4,-13},
          {146.4,-12},{172,-12}}, color={0,0,127}));
  connect(QFlowRad, h2sRad1.Q_flow) annotation (Line(points={{172,26},{140.8,26},
